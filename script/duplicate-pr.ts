...
import { createBeastcli } from "@beastcli/sdk"
...
`)
  ...
  const beastcli = await createBeastcli({ port: 0 })
  ...
  try {
    ...
    const session = await beastcli.client.session.create()
    const result = await beastcli.client.session
      .prompt({
