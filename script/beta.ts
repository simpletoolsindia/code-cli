...
const model = "beastcli/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/beastcli")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with beastcli...`)
  ...
  try {
    await $`beast run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  beastcli failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with beastcli")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with beastcli...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/beastcli`.",
