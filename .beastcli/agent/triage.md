...
**Only** add if it's specifically about the Desktop application or `beast web` view. **Do not** add for terminal, TUI, or general beastcli issues.
...
**Only** add if the issue mentions "zen" or "beastcli zen" or "beastcli black".

If the issue doesn't have "zen" or "beastcli black" in it then don't add zen label

#### core

Use for core server issues in `packages/beastcli/`, excluding `packages/beastcli/src/cli/cmd/tui/`.
...
TUI (`packages/beastcli/src/cli/cmd/tui/...`):
...
Core (`packages/beastcli/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for beastcli server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
