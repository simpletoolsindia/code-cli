...
import { createBeastcli, type ToolPart } from "@beastcli/sdk"
...
console.log("🚀 Starting beastcli server...")
const beastcli = await createBeastcli({
  port: 0,
})
console.log("✅ BeastCLI server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await beastcli.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new beast session...")
    const { client, server } = beastcli
    ...
    console.log("✅ Created beast session:", createResult.data.id)
  ...
  console.log("📝 Sending to beastcli:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 BeastCLI response:", JSON.stringify(result, null, 2))
