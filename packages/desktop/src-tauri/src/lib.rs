...
async fn initialize(app: AppHandle) {
    ...
    let sqlite_done = needs_migration.then(|| {
        tracing::info!(
            path = %beastcli_db_path().expect("failed to get db path").display(),
...
fn get_sidecar_port() -> u32 {
    option_env!("BEAST_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("BEAST_PORT").ok())
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = beastcli_db_path() else {
        return true;
    };
...
fn beastcli_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("beast").join("beastcli.db"))
}
