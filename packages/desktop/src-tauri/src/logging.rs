...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("beastcli-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("beastcli_lib=debug,beastcli_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("beastcli_lib=info,beastcli_desktop=info,sidecar=info")
        }
    });
