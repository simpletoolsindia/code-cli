...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("beastcli://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("beastcli://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("beastcli://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("beastcli://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("beastcli://open-project")).toBeUndefined()
    expect(parseDeepLink("beastcli://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "beastcli://open-project?directory=/a",
      "beastcli://other?directory=/b",
      "beastcli://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("beastcli://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("beastcli://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("beastcli://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("beastcli://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "beastcli://new-session?directory=/a",
      "beastcli://open-project?directory=/b",
      "beastcli://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __BEASTCLI__: {
        deepLinks: ["beastcli://open-project?directory=/a"],
      },
    } as unknown as Window & { __BEASTCLI__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["beastcli://open-project?directory=/a"])
