...
const reg = process.env.REGISTRY ?? "ghcr.io/simpletoolsindia"
...
const setup = async () => {
  ...
  if (list.includes("beast")) {
    await $`docker buildx use beastcli`
    return
  }
  await $`docker buildx create --name beastcli --use`
}
