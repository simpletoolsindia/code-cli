---
title: Providers
description: Using any LLM provider in BeastCLI.
---

import config from "../../../config.mjs"
export const console = config.console

BeastCLI uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your BeastCLI config.
...
in `~/.local/share/beastcli/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your BeastCLI
config.
...
BeastCLI Zen is a list of models provided by the BeastCLI team that have been
tested and verified to work well with BeastCLI. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with BeastCLI Zen.
:::

1. Run the `/connect` command in the TUI, select `BeastCLI Zen`, and head to [beastcli.ai/auth](https://beastcli.ai/zen).
...
It works like any other provider in BeastCLI and is completely optional to use.

---

## BeastCLI Go

BeastCLI Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the BeastCLI team that have been
tested and verified to work well with BeastCLI.

1. Run the `/connect` command in the TUI, select `BeastCLI Go`, and head to [beastcli.ai/auth](https://beastcli.ai/zen).
...
To use Amazon Bedrock with BeastCLI:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running beastcli:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY beastcli

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile beastcli

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX beastcli
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `beastcli.json`:

   ```json title="beastcli.json"
   {
     "$schema": "https://beastcli.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
BeastCLI. Anthropic explicitly prohibits this.

Previous versions of BeastCLI came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in BeastCLI with zero setup:
...
You can configure beastcli to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="beastcli.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://beastcli.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for beastcli to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX beastcli
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX beastcli
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your beastcli config.
...
GitLab Duo support in BeastCLI is experimental. Features, configuration, and
behavior may change in future releases.
:::

BeastCLI integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in beast auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
BeastCLI uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock BeastCLI
to only use your own GitLab-hosted instance, add the following to your
`beastcli.json` file. It is also recommended to disable session sharing.
...
More documentation on [beastcli-gitlab-auth](https://www.npmjs.com/package/beastcli-gitlab-auth) homepage.

##### Configuration

Customize through `beastcli.json`:
...
When a `duo-workflow-*` model is selected, BeastCLI will:
...
4. Route tool execution requests through BeastCLI's permission-gated tool system
...
To use your GitHub Copilot subscription with beastcli:
...
To use Google Vertex AI with BeastCLI:
...
2. Set the required environment variables:
   ...
   Set them while running beastcli.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id beastcli
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through beastcli, you can always configure it yourself.
...
```jsonc title="~/.config/beastcli/beastcli.jsonc"
{
  "$schema": "https://beastcli.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [beastcli-helicone-session](https://github.com/H2Shami/beastcli-helicone-session) plugin to automatically log each BeastCLI conversation as a session in Helicone.

```bash
npm install -g beastcli-helicone-session
```
...
```json title="beastcli.json"
{
  "plugin": ["beastcli-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each BeastCLI conversation listed as a separate session.
...
You can configure beastcli to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="beastcli.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://beastcli.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure beastcli to use local models through LM Studio.

```json title="beastcli.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://beastcli.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure beastcli to use local models through Ollama.

:::tip
Ollama can automatically configure itself for BeastCLI. See the [Ollama integration docs](https://docs.ollama.com/integrations/beastcli) for details.
:::

```json title="beastcli.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://beastcli.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with BeastCLI:
...
3. Copy the API key for use in BeastCLI.
...
6. **Important**: Before using cloud models in BeastCLI, you must pull the model information locally:
...
BeastCLI Zen is a list of tested and verified models provided by the BeastCLI team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>BeastCLI Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **BeastCLI Zen**.

   ```txt
   /connect
   ```

3. Enter your BeastCLI API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your beastcli config.
...
5. You can also customize them through your beastcli config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' beastcli
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group beastcli
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Beastcli:
...
You can also customize models through your beastcli config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with beastcli. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in beastcli.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `beastcli.json` file in your project directory:

   ```json title="beastcli.json" ""myprovider"" {5-15}
   {
     "$schema": "https://beastcli.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="beastcli.json" {9,11,17-20}
{
  "$schema": "https://beastcli.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow BeastCLI to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `beast auth list` to see if the credentials
...
2. For custom providers, check the beastcli config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your beastcli config.
