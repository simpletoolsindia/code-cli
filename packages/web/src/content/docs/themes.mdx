...
With BeastCLI you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, BeastCLI uses our own `beast` theme.
...
BeastCLI comes with several built-in themes.
...
- Want BeastCLI to match their terminal's appearance
...
BeastCLI supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/beastcli/themes/*.json` or `$XDG_CONFIG_HOME/beastcli/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.beastcli/themes/*.json`
4. **Current working directory** - Defined in `./.beastcli/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/beastcli/themes
vim ~/.config/beastcli/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .beastcli/themes
vim .beastcli/themes/my-theme.json
```
...
{
  "$schema": "https://beastcli.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
