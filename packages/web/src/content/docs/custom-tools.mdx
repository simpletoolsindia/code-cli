---
title: Custom Tools
description: Create tools the LLM can call in beastcli.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside beastcli's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.beastcli/tools/` directory of your project.
- Or globally, by placing them in `~/.config/beastcli/tools/`.
...
```ts title=".beastcli/tools/database.ts" {1}
...
```ts title=".beastcli/tools/math.ts"
import { tool } from "@beastcli/plugin"
...
```ts title=".beastcli/tools/bash.ts"
import { tool } from "@beastcli/plugin"
...
```ts title=".beastcli/tools/project.ts" {8}
import { tool } from "@beastcli/plugin"
...
```python title=".beastcli/tools/add.py"
import sys
...
```ts title=".beastcli/tools/python-add.ts" {10}
import { tool } from "@beastcli/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".beastcli/tools/add.py")
