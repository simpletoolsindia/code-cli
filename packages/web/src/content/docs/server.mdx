---
title: Server
description: Interact with beastcli server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `beast serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an beastcli client can use.
...
```bash
beast serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Custom domain name for mDNS service | `beastcli.local` |
| `--cors`        | Additional browser origins to allow | `[]`             |

`--cors` can be passed multiple times:

```bash
beast serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `BEAST_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `beast`, or set `BEAST_SERVER_USERNAME` to override it. This applies to both `beast serve` and `beast web`.

```bash
BEAST_SERVER_PASSWORD=your-password beast serve
```
...
When you run `beast` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the beastcli server to interact with beastcli programmatically.
:::

This architecture lets beastcli support multiple clients and allows you to interact with beastcli programmatically.

You can run `beast serve` to start a standalone server. If you have the
beastcli TUI running, `beast serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the BeastCLI [IDE](/docs/ide) plugins.
...
The beastcli server exposes the following APIs.
