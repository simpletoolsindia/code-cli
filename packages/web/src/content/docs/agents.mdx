...
There are two types of agents in BeastCLI; primary agents and subagents.
...
BeastCLI comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
BeastCLI comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

BeastCLI comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `beastcli.json` config file:
...
- Global: `~/.config/beastcli/agents/`
- Per-project: `.beastcli/agents/`

```markdown title="~/.config/beastcli/agents/review.md"
---
...
If no temperature is specified, BeastCLI uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global BeastCLI config and the project specific config.
...
The model ID in your BeastCLI config uses the format `provider/model-id`. For example, if you're using [BeastCLI Zen](/docs/zen), you would use `beastcli/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="beastcli.json" {3-6,9-12}
{
  "$schema": "https://beastcli.ai/config.json",
  "tools": {
    "write": true,
...
```json title="beastcli.json" {3-5,8-10}
{
  "$schema": "https://beastcli.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `beast models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/simpletoolsindia/code-cli).
:::
...
```markdown title="~/.config/beastcli/agents/docs-writer.md"
---
...
```markdown title="~/.config/beastcli/agents/security-auditor.md"
---
