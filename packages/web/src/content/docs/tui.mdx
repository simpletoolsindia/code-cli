---
title: TUI
description: Using the BeastCLI terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

BeastCLI provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running BeastCLI starts the TUI for the current directory.
...
When using the BeastCLI TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to BeastCLI. Allows you to select from available providers and add their API keys.
...
Exit BeastCLI. _Aliases_: `/quit`, `/q`
...
This is separate from `beastcli.json`, which configures server/runtime behavior.
...
Use `BEAST_TUI_CONFIG` to load a custom TUI config path.
