...
Tools allow the LLM to perform actions in your codebase. BeastCLI comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in BeastCLI.
...
:::note
This tool is only available when `BEAST_EXPERIMENTAL_LSP_TOOL=true` (or `BEAST_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the BeastCLI provider or when the `BEAST_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching BeastCLI:

```bash
BEAST_ENABLE_EXA=1 beastcli
```
