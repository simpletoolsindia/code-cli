---
title: SDK
description: Type-safe JS client for beastcli server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The beastcli JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control beastcli programmatically.
...
Create an instance of beastcli:

```javascript
import { createBeastcli } from "@beastcli/sdk"

const { client } = await createBeastcli()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `beastcli.json`, but you can override or add configuration inline:
...
console.log(`Server running at ${beastcli.server.url}`)
...
If you already have a running instance of beastcli, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@beastcli/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*beastcli" },
})
