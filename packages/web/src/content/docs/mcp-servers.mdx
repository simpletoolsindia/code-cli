...
You can add external tools to BeastCLI using the _Model Context Protocol_, or MCP. BeastCLI supports both local and remote servers.
...
You can define MCP servers in your [BeastCLI Config](https://beastcli.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="beastcli.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/beastcli` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
BeastCLI automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, BeastCLI will:
...
If the server requires authentication, BeastCLI will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `beast mcp auth <server-name>`.
...
```bash
beast mcp auth my-oauth-server
```
...
```bash
beast mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, BeastCLI will store the tokens securely in `~/.local/share/beastcli/mcp-auth.json`.
...
beast mcp debug my-oauth-server
```
...
Your MCPs are available as tools in BeastCLI, alongside built-in tools. So you can manage them through the BeastCLI config like any other tool.
...
```json title="beastcli.json" {11, 14-18}
{
  "$schema": "https://beastcli.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
beast mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect BeastCLI to your Sentry account.
