...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.beastcli.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `beastcli.settings.dat` is special: it maps to the `beastcli.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"beastcli.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // beastcli.settings.dat → the electron settings store ("beastcli.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "beastcli.global.dat").
  const storeName = filename === "beastcli.settings.dat" ? "beastcli.settings" : filename
