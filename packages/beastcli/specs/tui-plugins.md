...
- Author package entrypoint is `@beastcli/plugin/tui`.
...
```json
{
  "$schema": "https://beastcli.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/beastcli-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@beastcli/plugin/tui`.
- `@beastcli/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running BeastCLI version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.beastcli` is absent, no check is performed (backward compatible).
...
- `beastcli plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `beast plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.beastcli` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.beastcli`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.beastcli`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.beastcli` dir or `BEAST_CONFIG_DIR`, BeastCLI installs `@beastcli/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@beastcli/plugin/tui`.
...
- Local plugins persist installed themes under the local `.beastcli/themes` area near the plugin config source.
...
- `--pure` / `BEAST_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.beastcli/plugins/tui-smoke.tsx`
- Local vim plugin: `.beastcli/plugins/tui-vim.tsx`
- Local smoke config: `.beastcli/tui.json`
- Local smoke theme: `.beastcli/plugins/smoke-theme.json`
