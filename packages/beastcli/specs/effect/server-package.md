...
Practical reference for extracting a future `packages/server` from the current `packages/beastcli` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all beastcli services, Effect-first source of truth
- `packages/server` - beastcli server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full beastcli server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/beastcli`.
...
- those slices are mounted into the Hono server behind `BEAST_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `beast` package split is earlier in the migration.
...
- the current server host still lives in `packages/beastcli`
...
- if `packages/server` imports services from `packages/beastcli` while `packages/beastcli` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/beastcli` as the temporary Hono host
...
- `packages/server` must not import from `packages/beastcli`

Allowed in phase 1:

- `packages/beastcli` imports `packages/server`
...
- `packages/beastcli` shrinks or disappears as package responsibilities are fully split
...
- `packages/beastcli` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/beastcli`
...
Keep in `packages/beastcli` for now:
...
- host mounting from `packages/beastcli`
...
- no imports from `packages/beastcli` inside `packages/server`
...
- `packages/server` must still not import from `packages/beastcli`
...
### PR 4. Mount `packages/server` question from `packages/beastcli`

Scope:

- replace local experimental question route wiring in `packages/beastcli`
...
- mount it in parallel from `packages/beastcli`
...
- move adapter selection and server startup out of `packages/beastcli`

Rules:

- do not start this while `packages/server` still depends on `packages/beastcli`
...
- host logic no longer lives in `packages/beastcli`
...
3. the route is mounted from the current host in `packages/beastcli`
...
- `packages/server` imports services or runtime from `packages/beastcli`
- `packages/beastcli` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/beastcli` imports
...
- [ ] mount `question` from `packages/beastcli`
...
2. keep `packages/beastcli` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/beastcli`, stop and restructure the boundary first.
