...
Replace runtime Bun shell template-tag usage in `packages/beastcli/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/beastcli/src` is removed except:
