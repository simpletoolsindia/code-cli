    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        const client = createBeastcliClient({
          ...
          headers: Flag.BEAST_SERVER_PASSWORD
            ? {
                Authorization: `Basic ${Buffer.from(`${Flag.BEAST_SERVER_USERNAME ?? "beast"}:${Flag.BEAST_SERVER_PASSWORD}`).toString("base64")}`,
              }
            : undefined,
        ...
        const plugins = Flag.BEAST_PURE ? [] : (cfg.plugin_origins ?? [])
        if (Flag.BEAST_PURE && cfg.plugin_origins?.length) {
