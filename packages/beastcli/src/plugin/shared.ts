...
export async function checkPluginCompatibility(target: string, beastcliVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(beastcliVersion) || semver.major(beastcliVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.beastcli
  if (typeof range !== "string") return
  if (!semver.satisfies(beastcliVersion, range)) {
    throw new Error(`Plugin requires beastcli ${range} but running ${beastcliVersion}`)
  }
}
