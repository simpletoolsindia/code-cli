...
import { BeastcliHttpApi } from "./api"
...
const LegacyComponentDescriptions = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for beast serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw = OpenApi.fromApi(BeastcliHttpApi) as unknown as OpenApiSpec
...
export const PublicApi = BeastcliHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "beast",
    version: "1.0.0",
    description: "beastcli api",
