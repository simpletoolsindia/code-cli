...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for beastcli.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal beastcli sessions
...
```bash
BEAST_ENABLE_QUESTION_TOOL=1 beast acp
```
...
- **Terminal Integration**: Full terminal support via beastcli's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | beast acp
```
...
ACP sessions map cleanly to beastcli's internal session model:
