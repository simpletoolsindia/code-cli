...
export async function init({ sdk: _sdk }: { sdk: BeastcliClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `beast auth login` in the terminal",
      name: "Login with beastcli",
      id: "beastcli-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // BeastCLI stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const beastcliProvider = providers.find((p) => p.id === "beast")
  if (beastcliProvider) {
    if (beastcliProvider.models["big-pickle"]) {
      return { providerID: ProviderID.beastcli, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(beastcliProvider.models))
    if (best) {
      return {
