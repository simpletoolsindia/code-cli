...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in beastcli config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "beastcli.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "beastcli.jsonc")))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "BEAST_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/beastcli` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/beastcli`))
            if (!response.ok) {
        ...
        if (Flag.BEAST_CONFIG) {
          yield* merge(Flag.BEAST_CONFIG, yield* loadFile(Flag.BEAST_CONFIG))
          log.debug("loaded custom config", { path: Flag.BEAST_CONFIG })
        }
        ...
        if (Flag.BEAST_CONFIG_DIR) {
          log.debug("loading config from BEAST_CONFIG_DIR", { path: Flag.BEAST_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".beastcli") || dir === Flag.BEAST_CONFIG_DIR) {
            for (const file of ["beastcli.json", "beastcli.jsonc"]) {
          ...
          // Auto-discovered plugins under `.beastcli/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.BEAST_CONFIG_CONTENT) {
          const source = "BEAST_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.BEAST_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from BEAST_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["BEAST_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("BEAST_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.BEAST_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.BEAST_PERMISSION))
        }
        ...
        if (Flag.BEAST_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.BEAST_DISABLE_PRUNE) {
