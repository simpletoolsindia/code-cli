...
const MANAGED_PLIST_DOMAIN = "ai.beastcli.managed"

// Keys injected by macOS/MDM into the managed plist that are not BeastCLI config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/beastcli"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "beast")
    default:
      return "/etc/beastcli"
  }
}
...
export function managedConfigDir() {
  return process.env.BEAST_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
