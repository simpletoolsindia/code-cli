...
const AGENT_USERNAME = "beastcli-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/beastcli.yml"
      ...
      async fn() {
        {
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/beastcli-agent"
            const command =
...
jobs:
  beastcli:
    if: |
      ...
      startsWith(github.event.comment.body, '/beastcli')
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/beastcli,/oc")
          .split(",")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("beastcli-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `beastcli/${type}-${hex}-${timestamp}`
        }
        return `beastcli/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/beastcli-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[beast session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the beastcli infrastructure after your response",
