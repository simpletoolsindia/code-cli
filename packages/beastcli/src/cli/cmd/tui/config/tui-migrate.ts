...
const TUI_SCHEMA_URL = "https://beastcli.sridharhomelab.in/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from beastcli.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const beastcli = await beastcliFiles(input)
  for (const file of beastcli) {
...
async function beastcliFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "beast"),
    ...(await Filesystem.findUp(["beastcli.json", "beastcli.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "beast"))
  }
  if (Flag.BEAST_CONFIG) files.push(Flag.BEAST_CONFIG)
