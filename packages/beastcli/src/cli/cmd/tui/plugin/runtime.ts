...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@simpletoolsindia/plugin/tui"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".beastcli"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".beastcli", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".beastcli", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await Instance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.BEAST_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.BEAST_PURE && config.plugin_origins?.length) {
