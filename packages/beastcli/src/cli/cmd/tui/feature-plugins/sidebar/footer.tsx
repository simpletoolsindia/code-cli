...
function View(props: { api: TuiPluginApi }) {
  ...
  const has = createMemo(() =>
    props.api.state.provider.some(
      (item) => item.id !== "beast" || Object.values(item.models).some((model) => model.cost?.input !== 0),
    ),
  )
      ...
      <Show when={show()}>
        ...
        >
          ...
          <box flexGrow={1} gap={1}>
            ...
            <text fg={theme().textMuted}>BeastCLI includes free models so you can start immediately.</text>
