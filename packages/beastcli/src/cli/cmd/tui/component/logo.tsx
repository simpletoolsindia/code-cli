...
function lit(char: string, encoded?: boolean) {
  if (!encoded) return char !== " "
...
function build(shape: LogoShape): LogoContext {
  ...
  return { LEFT, FULL, SPAN, MAP: mapGlyphs(FULL, shape.encoded), shape }
}
...
export function Logo(props: { shape?: LogoShape; ink?: RGBA; idle?: boolean } = {}) {
  ...
  const renderLine = (
    ...
    return Array.from(line).map((char, i) => {
      ...
      const charLit = lit(char, ctx.shape.encoded)
