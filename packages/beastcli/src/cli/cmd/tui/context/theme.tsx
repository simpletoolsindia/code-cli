...
import beastcli from "./theme/beastcli.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@simpletoolsindia/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "beastcli")
        draft.active = typeof active === "string" ? active : "beastcli"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.beastcli, store.mode)
    })
