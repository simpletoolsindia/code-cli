...
import { DialogKeybinds } from "./ui/dialog-keybinds"
import { OnboardingTour } from "@tui/component/onboarding-tour"
...
import { ToastHistoryDialog } from "@tui/ui/dialog-toast-history"
...
function rendererConfig(_config: TuiConfig.Info): CliRendererConfig {
  const mouseEnabled = !Flag.BEAST_DISABLE_MOUSE && (_config.mouse ?? true)
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  useKeyboard((evt) => {
    if (!Flag.BEAST_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.BEAST_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("BeastCLI")
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`BeastCLI | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`BeastCLI | ${route.data.id}`)
    }
  })
  ...
  createEffect(
    on(
      ...
      (complete, wasComplete) => {
        ...
        if (kv.get("onboarding_completed")) return
  ...
  command.register(() => [
    ...
    },
    {
      ...
      onSelect: () => {
        dialog.replace(() => <ToastHistoryDialog />)
      },
    },
    {
      title: "Replay onboarding tour",
      value: "onboarding.replay",
      category: "System",
      slash: {
        name: "tour",
        aliases: ["onboarding"],
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to BeastCLI v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={Flag.BEAST_EXPERIMENTAL_DISABLE_COPY_ON_SELECT ? undefined : () => Selection.copy(renderer, toast)}
    >
      <Show when={Flag.BEAST_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
        <Switch>
