...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall beastcli and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall BeastCLI")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g beastcli-ai",
      pnpm: "pnpm uninstall -g beastcli-ai",
      bun: "bun remove -g beastcli-ai",
      yarn: "yarn global remove beastcli-ai",
      brew: "brew uninstall beastcli",
      choco: "choco uninstall beastcli",
      scoop: "scoop uninstall beastcli",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string[]> = {
      ...
      bun: ["bun", "remove", "-g", "beastcli-ai"],
      yarn: ["yarn", "global", "remove", "beastcli-ai"],
      brew: ["brew", "uninstall", "beast"],
      choco: ["choco", "uninstall", "beast"],
      scoop: ["scoop", "uninstall", "beast"],
    }

    const cmd = cmds[method]
    if (cmd) {
      spinner.start(`Running ${cmd.join(" ")}...`)
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "beast", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using BeastCLI!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# beastcli") || content.includes(".beastcli/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".beastcli/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".beastcli/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".beastcli"))
    ) {
      continue
    }
