...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run beastcli",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for beast session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found beast session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["beast", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting beastcli...")
        UI.println()

        const beastcliArgs = sessionId ? ["-s", sessionId] : []
        const beastcliProcess = Process.spawn(["beast", ...beastcliArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await beastcliProcess.exited
        if (code !== 0) throw new Error(`beastcli exited with code ${code}`)
      },
    })
  },
})
