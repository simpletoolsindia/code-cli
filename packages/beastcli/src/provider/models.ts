...
function url() {
  return Flag.BEAST_MODELS_URL || "https://models.dev"
}
...
export const Data = lazy(async () => {
  const result = await Filesystem.readJson(Flag.BEAST_MODELS_PATH ?? filepath).catch(() => {})
  ...
  if (Flag.BEAST_DISABLE_MODELS_FETCH) return {}
...
if (!Flag.BEAST_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
  void refresh()
  setInterval(
    async () => {
      await refresh()
