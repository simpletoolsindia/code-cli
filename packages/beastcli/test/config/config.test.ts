...
const managedConfigDir = process.env.BEAST_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "beastcli.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "beastcli.json") {
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "beastcli.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "beastcli.jsonc")
...
test("ignores legacy tui keys in beastcli config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://beastcli.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "beastcli.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["BEAST_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { beastcli: { options: { apiKey: "{env:BEAST_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["beast"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.provide(Npm.defaultLayer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["BEAST_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["BEAST_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "beastcli.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .beastcli directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const beastcliDir = path.join(dir, ".beastcli")
      await fs.mkdir(beastcliDir, { recursive: true })
      const agentDir = path.join(beastcliDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".beastcli", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .beastcli/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const beastcliDir = path.join(dir, ".beastcli")
      await fs.mkdir(beastcliDir, { recursive: true })

      const agentsDir = path.join(beastcliDir, "agents")
...
test("loads commands from .beastcli/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const beastcliDir = path.join(dir, ".beastcli")
      await fs.mkdir(beastcliDir, { recursive: true })

      const commandDir = path.join(beastcliDir, "command")
...
test("loads commands from .beastcli/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const beastcliDir = path.join(dir, ".beastcli")
      await fs.mkdir(beastcliDir, { recursive: true })

      const commandsDir = path.join(beastcliDir, "commands")
...
test("does not try to install dependencies in read-only BEAST_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.BEAST_CONFIG_DIR
  process.env.BEAST_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.BEAST_CONFIG_DIR
    else process.env.BEAST_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable BEAST_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "beastcli.json"),
        JSON.stringify({ $schema: "https://beastcli.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .beastcli config
      const projectDir = path.join(dir, "project")
      const beastcliDir = path.join(projectDir, ".beastcli")
      await fs.mkdir(beastcliDir, { recursive: true })
      ...
      // Local .beastcli config with different plugins
      await Filesystem.write(
        path.join(beastcliDir, "beastcli.json"),
        JSON.stringify({
          $schema: "https://beastcli.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .beastcli config with some overlapping plugins
      await Filesystem.write(
        path.join(beastcliDir, "beastcli.json"),
        JSON.stringify({
          $schema: "https://beastcli.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".beastcli")
...
// Note: preload.ts sets BEAST_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .beastcli config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .beastcli directory config enables it
      const beastcliDir = path.join(dir, ".beastcli")
      await fs.mkdir(beastcliDir, { recursive: true })
      await Filesystem.write(
        path.join(beastcliDir, "beastcli.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/beastcli")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/beastcli")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "beastcli.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-beastcli@2.4.3", file)).toBe("oh-my-beastcli@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-beastcli@2.4.3", "file:///project/.beastcli/plugin/oh-my-beastcli.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.beastcli/plugin/demo.ts", "file:///project/.beastcli/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.beastcli/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const beastcliDir = path.join(projectDir, ".beastcli")
        const pluginDir = path.join(beastcliDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("BEAST_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["BEAST_DISABLE_PROJECT_CONFIG"]
    process.env["BEAST_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["BEAST_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["BEAST_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .beastcli/ directories when flag is set", async () => {
    const originalEnv = process.env["BEAST_DISABLE_PROJECT_CONFIG"]
    process.env["BEAST_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .beastcli directory with a command
          const beastcliDir = path.join(dir, ".beastcli", "command")
          await fs.mkdir(beastcliDir, { recursive: true })
          await Filesystem.write(path.join(beastcliDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .beastcli should NOT be in directories list
          const hasProjectBeastcli = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectBeastcli).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["BEAST_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["BEAST_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["BEAST_CONFIG_DIR"]
      process.env["BEAST_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["BEAST_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["BEAST_CONFIG_DIR"]
      } else {
        process.env["BEAST_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("BEAST_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["BEAST_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["BEAST_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["BEAST_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["BEAST_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from BEAST_CONFIG_DIR, not project
...
describe("BEAST_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in BEAST_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["BEAST_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["BEAST_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://beastcli.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["BEAST_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["BEAST_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in BEAST_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["BEAST_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.effectSchema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "BeastCLI Managed",
          PayloadIdentifier: "ai.beastcli.managed.test",
          PayloadType: "ai.beastcli.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://beastcli.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://beastcli.ai/config.json")
})
