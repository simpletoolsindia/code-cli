...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "beastcli.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "beastcli.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "beastcli.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "beastcli.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["beastcli.json", "beastcli.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "beastcli.json"),
        path.join(tmp.path, "beastcli.jsonc"),
        path.join(project, "beastcli.json"),
        path.join(project, "beastcli.jsonc"),
      ])
    })
  })
