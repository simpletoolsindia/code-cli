...
const originalBeastcliPort = process.env.BEAST_EDITOR_SSE_PORT

afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort
  process.env.BEAST_EDITOR_SSE_PORT = originalBeastcliPort
})
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.BEAST_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with BEAST_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.BEAST_EDITOR_SSE_PORT = "4020"
