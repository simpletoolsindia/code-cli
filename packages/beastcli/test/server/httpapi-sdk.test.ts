...
type Sdk = ReturnType<typeof createBeastcliClient>
...
function app(backend: Backend, input?: { password?: string; username?: string }) {
  Flag.BEAST_EXPERIMENTAL_HTTPAPI = backend === "httpapi"
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("beast", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("beast", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
