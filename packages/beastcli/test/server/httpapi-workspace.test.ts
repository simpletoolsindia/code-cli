...
const originalHttpApi = Flag.BEAST_EXPERIMENTAL_HTTPAPI
const it = testEffect(
...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-beastcli-directory", directory)
...
afterEach(async () => {
  ...
  Flag.BEAST_EXPERIMENTAL_HTTPAPI = originalHttpApi
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-beastcli-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://beastcli.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-beastcli-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-beastcli-workspace")
      } finally {
