...
const original = Flag.BEAST_EXPERIMENTAL_HTTPAPI
...
afterEach(async () => {
  Flag.BEAST_EXPERIMENTAL_HTTPAPI = original
...
describe("instance HttpApi", () => {
  test("serves catalog read endpoints through Hono bridge", async () => {
    ...
    const [commands, agents, skills, lsp, formatter] = await Promise.all([
      app().request(InstancePaths.command, { headers: { "x-beastcli-directory": tmp.path } }),
      app().request(InstancePaths.agent, { headers: { "x-beastcli-directory": tmp.path } }),
      app().request(InstancePaths.skill, { headers: { "x-beastcli-directory": tmp.path } }),
      app().request(InstancePaths.lsp, { headers: { "x-beastcli-directory": tmp.path } }),
      app().request(InstancePaths.formatter, { headers: { "x-beastcli-directory": tmp.path } }),
    ])
  ...
  test("serves project git init through Hono bridge", async () => {
    ...
    const response = await app().request("/project/git/init", {
      method: "POST",
      headers: { "x-beastcli-directory": tmp.path },
    })
    ...
    const current = await app().request("/project/current", { headers: { "x-beastcli-directory": tmp.path } })
  ...
  test("serves project update through Hono bridge", async () => {
    ...
    const response = await app().request(`/project/${project.id}`, {
      method: "PATCH",
      headers: { "x-beastcli-directory": tmp.path, "content-type": "application/json" },
    ...
    const list = await app().request("/project", { headers: { "x-beastcli-directory": tmp.path } })
