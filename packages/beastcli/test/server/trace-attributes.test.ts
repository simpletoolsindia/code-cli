...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under beastcli.", () => {
    expect(paramToAttributeKey("name")).toBe("beastcli.name")
    expect(paramToAttributeKey("slug")).toBe("beastcli.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["beastcli.sessionID"]).toBeUndefined()
    expect(attrs["beastcli.messageID"]).toBeUndefined()
    expect(attrs["beastcli.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under beastcli. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["beastcli.name"]).toBe("exa")
