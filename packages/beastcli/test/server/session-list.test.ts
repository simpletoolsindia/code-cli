...
const originalWorkspaces = Flag.BEAST_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.BEAST_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session.list", () => {
  test("does not filter by directory when directory is omitted", async () => {
    ...
    await mkdir(path.join(tmp.path, "packages", "beast"), { recursive: true })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const current = await Instance.provide({
          directory: path.join(tmp.path, "packages", "beast"),
  ...
  test("filters by directory when directory is provided", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const ids = (await svc.list({ directory: path.join(tmp.path, "packages", "beast") })).map((s) => s.id)
  ...
  test("filters by path and ignores directory when path is provided", async () => {
    Flag.BEAST_EXPERIMENTAL_WORKSPACES = false
    await using tmp = await tmpdir({ git: true })
    await mkdir(path.join(tmp.path, "packages", "beast", "src", "deep"), { recursive: true })
    await mkdir(path.join(tmp.path, "packages", "app"), { recursive: true })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const deeper = await Instance.provide({
          directory: path.join(tmp.path, "packages", "beast", "src", "deep"),
  ...
  test("falls back to directory when filtering legacy sessions without path", async () => {
    Flag.BEAST_EXPERIMENTAL_WORKSPACES = false
    await using tmp = await tmpdir({ git: true })
    await mkdir(path.join(tmp.path, "packages", "beast", "src"), { recursive: true })
    await mkdir(path.join(tmp.path, "packages", "app"), { recursive: true })
