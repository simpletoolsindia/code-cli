...
describe("installation", () => {
  describe("latest", () => {
    ...
    test("reads npm versions via registry", async () => {
      ...
      expect(calls).toContain(`https://registry.npmjs.org/beastcli-ai/${InstallationChannel}`)
    })
    ...
    test("reads brew formulae API versions", async () => {
      const layer = testLayer(
        ...
        (cmd, args) => {
          ...
          if (cmd === "brew" && args.includes("--formula") && args.includes("simpletoolsindia/tap/code-cli")) return ""
          if (cmd === "brew" && args.includes("--formula") && args.includes("beast")) return "beast"
          return ""
        },
      )
